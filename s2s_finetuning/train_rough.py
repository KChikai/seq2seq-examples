...
def main():
    ...
    if not os.path.exists('./data/corpus/input_mat.npy'):
        ...
        np.save('./data/corpus/input_mat.npy', input_mat)
        np.save('./data/corpus/output_mat.npy', output_mat)
    else:
        input_mat = np.load('./data/corpus/input_mat.npy')
        output_mat = np.load('./data/corpus/output_mat.npy')
    ...
    for num, epoch in enumerate(range(n_epoch)):
        ...
        batch_num = 0
        perm = np.random.permutation(len(corpus.rough_posts) - testsize)

        # for training
        for i in range(0, len(corpus.rough_posts) - testsize, batchsize):
