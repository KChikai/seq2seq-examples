...
from s2s_bidirectional.seq2seq_bk import Seq2Seq
...
def main():
    ...
    counts = [corpus.dic.token2id[token] for token in corpus.dic.token2id]

    ######################
    #### create model ####
    ######################

    sample_size = int(len(counts) * 0.005)
    print('sample size = ', sample_size)
    model = Seq2Seq(vocab_size=len(corpus.dic.token2id), feature_num=feature_num,
                    hidden_num=hidden_num, batch_size=batchsize, counts=counts,
                    sample_size=sample_size, gpu_flg=args.gpu)
    if args.gpu >= 0:
        model.to_gpu()
    ...
    for num, epoch in enumerate(range(n_epoch)):
        ...
        for i in range(0, len(corpus.posts) - testsize, batchsize):
            ...
            loss = model.decode(end_batch, first_words, train=True)
            next_ids = first_words
            accum_loss += loss
            for w_ids in output_batch[1:]:
                loss = model.decode(next_ids, w_ids, train=True)
                next_ids = w_ids
                accum_loss += loss
