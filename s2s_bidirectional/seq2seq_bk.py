...
class Seq2Seq(chainer.Chain):
    ...
    def __init__(self, vocab_size, feature_num, hidden_num, batch_size, counts, sample_size, gpu_flg):
        """
        ...
        super(Seq2Seq, self).__init__(
            ...
            dec=Decoder(vocab_size, feature_num, hidden_num * 2, counts, sample_size=sample_size)   # decoder
        )

    def encode(self, input_batch, input_batch_rev, train=True):
        """
    ...
    def decode(self, predict_id, teacher_id, train=True):
        """
        ...
        t = chainer.Variable(xp.array(teacher_id, dtype=xp.int32))
        if train:
            loss, self.c_batch, self.h_batch = self.dec(batch_word, t, self.c_batch, self.h_batch, self.h_enc, train)
            return loss
        else:
            predict_mat, self.c_batch, self.h_batch = self.dec(batch_word, t, self.c_batch, self.h_batch, self.h_enc, train)
            return predict_mat
    ...
    def one_decode(self, predict_id, teacher_id, train=False):
        """
        :param predict_id:
        ...
        t = chainer.Variable(xp.array(teacher_id, dtype=xp.int32))
        if train:
            predict_vec, self.c_batch, self.h_batch = self.dec(word, t, self.c_batch, self.h_batch, self.h_enc, train=train)
            return F.softmax_cross_entropy(predict_vec, t), predict_vec
        else:
            loss, self.c_batch, self.h_batch = self.dec(word, t, self.c_batch, self.h_batch, self.h_enc, train=train)
            return loss
